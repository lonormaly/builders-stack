...
// - Inline scripts that must run (app/auth/popup-complete) read the nonce from the
//   x-nonce request header via next/headers.
//
...
//
// ponytail: the CSP half of this file is duplicated in apps/landing (same rule as the
// headers() block in next.config.ts — centralize only if a 4th app appears).
